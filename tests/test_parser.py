...
def test_parses_mapping_name_and_target(mapping):
    assert mapping.name == "m_customers"
    assert mapping.target.name == "customers"


def test_parses_source_group_from_folder_name(mapping):
    assert mapping.source_group == "jaffle_shop"


def test_parses_sources(mapping):
...
def test_parses_target_fields_in_order(mapping):
...
def test_parses_all_transformations_with_correct_types(mapping):
    expected_types = {
...
def test_transformation_ports_carry_expression_info(mapping):
...
def test_aggregator_groupby_ports(mapping):
...
def test_joiner_table_attributes(mapping):
...
def test_instance_and_connector_counts(mapping):
    assert len(mapping.instances) == 16
    assert len(mapping.connectors) == 60


def test_a_specific_connector(mapping):
    match = [
        c
